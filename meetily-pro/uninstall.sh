#!/bin/bash
#
# Meetily Pro - Uninstaller
# Safely removes all Meetily Pro components
#
# Usage: curl -sSL https://meetily.pro/uninstall.sh | bash
#
...
print_warning() {
    ...
    echo "║                    ⚠️  WARNING                               ║"
    echo "║                                                              ║"
    echo "║  This will permanently remove Meetily Pro and ALL data       ║"
    echo "║  including:                                                  ║"
    echo "║    • Application files                                       ║"
    echo "║    • Database (all organizations, users, meetings)           ║"
    echo "║    • Meeting recordings and transcripts                      ║"
    echo "║    • Configuration files                                     ║"
    echo "║    • SSL certificates                                        ║"
    echo "║    • Log files and audit trails                              ║"
    echo "║                                                              ║"
...
confirm_uninstall() {
    echo ""
    read -p "Are you sure you want to uninstall Meetily Pro? (yes/no): " confirm
    if [[ "$confirm" != "yes" ]]; then
...
backup_before_uninstall() {
    ...
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        if docker ps 2>/dev/null | grep -q "meetily-postgres"; then
            log_info "Dumping database..."
            docker exec meetily-postgres pg_dumpall -U postgres > "$BACKUP_DIR/database.sql" 2>/dev/null || true
        fi
        
        # Backup storage
        if [[ -d "$INSTALL_DIR/storage" ]]; then
            log_info "Backing up storage..."
            tar czf "$BACKUP_DIR/storage.tar.gz" -C "$INSTALL_DIR" storage/ 2>/dev/null || true
        fi
...
remove_containers() {
    ...
    docker rm -f meetily-frontend meetily-backend meetily-postgres meetily-redis meetily-nginx meetily-minio meetily-certbot 2>/dev/null || true
...
remove_images() {
    ...
    docker rmi -f meetily-pro-frontend meetily-pro-backend 2>/dev/null || true
...
remove_systemd_service() {
    ...
    if [[ -f /etc/systemd/system/meetily-pro.service ]]; then
        systemctl stop meetily-pro.service 2>/dev/null || true
        systemctl disable meetily-pro.service 2>/dev/null || true
        rm -f /etc/systemd/system/meetily-pro.service
...
cleanup_networks() {
    ...
    docker network rm meetily-network 2>/dev/null || true
...
print_completion() {
    ...
    echo "  curl -sSL https://meetily.pro/uninstall.sh | bash"
}
